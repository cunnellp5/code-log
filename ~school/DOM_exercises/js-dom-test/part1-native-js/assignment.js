...
function createDivWithClass (className){
  var nillaScript = createEmptyDiv();
  nillaScript.classList += className;
  return nillaScript;
}
...
function updateTodoList (todoList){
  var items = todoList.getElementsByTagName('li');
   ...
   for (var i = 0; i <items.length; i++) {
     if (items[i].innerText.substring(0,9) == 'COMPLETED' ){
       todoList.removeChild(items[i]);
       i--;
     } else (items[i].innerText.substring(0,6) == 'URGENT')
...
function createList (object){
  var bigList = document.getElementById('a');
  var inputOne = document.createElement(object)

  // var b = document.createElement('li')
  // var link = document.createTextNode(object)
  // var new = newItem.appendChild(object);
  console.log(object);
}
